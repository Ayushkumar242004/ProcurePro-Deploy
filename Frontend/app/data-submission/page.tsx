...
export default function DataSubmissionPage() {
  ...
  useEffect( ( ) => { 
    const checkData = async ( ) => {
      try{
        const dbResponse = await fetch('https://procurepro-x4li.onrender.com/api/get-esg-prefill' , 
        {
          headers: { "email": email }
        }) ; 
  ...
  try {
    const response = await fetch("https://procurepro-x4li.onrender.com/api/calculate-and-store-esg", {
    method: "POST",
    headers: {
